...
params <- c(r=0.31271, C=100, d=0.001, k=0.235)#h g
...
params <- c(r=0.31271, C=100, d=0.001, k=0.235,Km=3.001*10^-7 ,Vmas=2.274*10^-4)
...
s<-ggplot(santaloldf,aes(x=Time))+
  ...
  scale_colour_manual("", 
                      breaks = c("Without transporter", "With transporter"),
                      values = c("#D81B60", "#2224f0")) +
  scale_x_continuous(expand = c(0, 0)) + scale_y_continuous(expand = c(0, 0),limits = c(0,600))+
  theme_classic()+
  xlab("Time(h)")+
  ylab("Total Ambrein Production(g)")+
  theme(axis.text.x = element_text(size = 12),
        axis.title.x = element_text(size = 16),
        axis.text.y =  element_text(size = 12),
        axis.title.y = element_text(size = 16),
